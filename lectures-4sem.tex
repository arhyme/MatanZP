...
То есть, всяка билинейная форма есть сумма симметричной и кососимметричной (первое и второе слагаемые соотвественно), где $l^T \langle u, v \rangle = l \langle v, u \rangle$:
...
\begin{lemma}
	...
	\begin{equation*}
		dw = \sum_{i=1}^{n} da_i \wedge dx_i = \sum_{i=1}^{n} (\sum_{j=1}^{n} \frac{\partial a_i}{\partial x_j} dx_j) \wedge dx_i
	\end{equation*}
\end{lemma}
...
%Лекция 16.04.19 TODO

%Лекция 18.04.19

%TODO Баяр опоздал на лекцию
...
\begin{proof}
	Проведем доказательство для куба в $\mathbb{R}^n$. Зададим куб как $[-a, a]^n$ и заметим, что он является $n$-мерным многообразием в $\mathbb{R}^n$ и у него есть $2n$ $(n-1)$-мерных граней
	...
	\begin{align*}
		dw(x) = \sum\limits_{j=1}^{n} da_j(x) (-1)^{j + 1} dx_1 \wedge \ldots \wedge \widehat{dx_j} \wedge \ldots \wedge dx_n = \\ =\sum\limits_{j=1}^{n} (\frac{\partial a_j}{\partial x_1} dx_1 + \ldots + \frac{\partial a_j}{\partial x_n} dx_n) \wedge (-1)^{j+1} dx_1 \wedge \ldots \wedge \widehat{dx_j} \wedge \ldots \wedge dx_n = \\ = \sum\limits_{j=1}^{n} \frac{\partial a_j}{\partial x_j} (-1)^{j+1} dx_j \wedge dx_1 \wedge \ldots \wedge \widehat{dx_j} \wedge \ldots \wedge dx_n = (\sum\limits_{j = 1}^{n} \frac{\partial a_j}{\partial x_j}) dx_1 \wedge \ldots \wedge dx_n
	\end{align*} 
	
	Вычислим интеграл от $dw$ по кубу
	
	\begin{equation*}
		\int\limits_{[-a, a]^k} dw = \int\limits_{[-a, a]^k} (\sum\limits_{j = 1}^{n} \frac{\partial a_j}{\partial x_j}) dx_1 \wedge \ldots \wedge dx_n = \int\limits_{-a}^{a} \ldots \int\limits_{-a}^{a} (\sum\limits_{j = 1}^{n} \frac{\partial a_j}{\partial x_j}) dx_1 \wedge \ldots \wedge dx_n
	\end{equation*}
	...
	\begin{equation*}
		\int\limits_{x_j = a} w + \int\limits_{x_j = -a} w = \int\limits_{-a}^{a} \ldots \int\limits_{-a}^{a} (a_j \bigg|_{x_j = a} - a_j \bigg|_{x_j = -a}) \ldots \widehat{dx_j}\ldots dx_n
	\end{equation*}
	...
	%TODO Тут какая то ересь написана
\end{proof}
...
\begin{definition}
	Форма $w$ называется коточной, если сущесвует форма $\theta$, такая что $w = I \theta$.
\end{definition}
...
Напомним, что через $S_r$ и $B_r$ мы обозначаем сферу и шар радиуса $r$ соотвественно.

\begin{definition}
	Дивергения векторного поля $\vec{v}$ в точке $x$ определяется как
	
	\begin{equation*}
		div \; \vec{v}(x) = \lim\limits_{r \to 0} \frac{1}{|B_r|} \int\limits_{S_r} \vec{v} \cdot \vec{n} \; dS^{n-1}
	\end{equation*}
...
\begin{lemma}[Оператор дивергенции в декартовых координатах]
	...
	\begin{equation*}
		div \; \vec{v}(x) = \sum\limits_{j = 1}^{n} \frac{\partial a_j}{\partial x_j}(x)
	\end{equation*}
\end{lemma}
...
\begin{theorem}[Формула Гаусса-Остроградского]
	...
	\begin{equation*}
		\int\limits_{\partial v} \vec{v}(x) \cdot \vec{n}(x) d S^{n-1} = \int\limits_{V} div \; \vec{x}(x) dx 
	\end{equation*}
\end{theorem}

\begin{definition}
	Векторное поле $\vec{v}$ называется \textit{бездивергентным}, если $div \; \vec{v} = 0$.
\end{definition}

Имееют место следующие соответствия, где соответствующие члены двух цепей изоморфны между собой:

\begin{align*}
	&Scal (\mathbb{R}^3) \xrightarrow[]{grad} Vec (\mathbb{R}^3) \xrightarrow[]{rot} Vec (\mathbb{R}^3) \xrightarrow[]{grad} Scal (\mathbb{R}^3) \\
	&\{U \to \Lambda^0\} \xrightarrow[]{d} \{U \to \Lambda^1\} \xrightarrow[]{d} \{U \to \Lambda^2\} \xrightarrow[]{d} \{U \to \Lambda^3\}
\end{align*}

То есть, на языке дифференциальных форма, каждой операции векторного анализа соотвествует дифференциал. 

В плоском случае можно получить следующие выражения:

\begin{align*}
	& rot (a, b) \simeq d (adx + bdy) = (\frac{\partial a}{\partial x} - \frac{\partial a}{\partial y})  dx \wedge dy = \bigtriangledown \wedge (a, b) =
	\begin{vmatrix}
		\frac{\partial}{\partial x} & \frac{\partial}{\partial y} \\
		a & b
	\end{vmatrix} \\
	&div (a, b) \simeq d(adx + bdy) = (\frac{\partial a}{\partial x} + \frac{\partial a}{\partial y})  dx \wedge dy = \bigtriangledown \cdot (a, b)
\end{align*}
...
Леммы ниже являются следствиями первой и второй теоремы Пуанкаре соотвественно.

\begin{lemma}
	Вихревое векторное поле всегда бездивергентно, т.е. $div \; rot \vec{v} = 0$.
\end{lemma}

\begin{lemma}
	Если векторное поле $\vec{v}$ в звездной области в $\mathbb{R}^3$ бездиверегентно, то она вихревое.
\end{lemma}
...
\begin{definition}
	Оператором Лапласа называется оператор $\bigtriangleup : Scal \to Scal$, такой, что 
	
	\begin{equation*}
		\bigtriangleup = \bigtriangledown^2 = div \; grad
	\end{equation*}
\end{definition}
...
\begin{equation*}
\bigtriangleup f = div (\frac{\partial f}{\partial x_1}, \ldots, \frac{\partial f}{\partial x_n}) = \frac{\partial^2 f}{\partial x_1^2} + \ldots + \frac{\partial^2 f}{\partial x_n^2}
\end{equation*}

Если $\bigtriangleup f > 0$, то $f(x)$ больше своего среднего значения на маленькой сфере с центром в точке $x$.

Если $\bigtriangleup f < 0$, то $f(x)$ меньше своего среднего значения на маленькой сфере с центром в точке $x$.

Если $\bigtriangleup f = 0$, то $f(x)$ приблизительно равна своему среднему значению на маленькой сфере с центром в точке $x$.
