...
\begin{example}\label{v-2N1c}
	...
	тогда
	\begin{equation*}
		J_\varphi(t) = \det \sqrt{D\varphi^*(t)D\varphi(t)} = \det
		\begin{pmatrix}
		1 & 0 \\
		0 & t^2 + 1
		\end{pmatrix}^{1/2}
	...
	\begin{eqnarray}\nonumber
		...
		t\sqrt{t^2 + 1} + \mathrm{arcsinh}(p) \right) \bigg|_0^{\arctan(2)} =\\\nonumber \frac{\pi}{4} \left( \arctan(2)\sqrt{\arctan^2(2) + 1} +  \mathrm{arcsinh}(\arctan(2))\right)
	\end{eqnarray}
