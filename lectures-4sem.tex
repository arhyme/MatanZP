...
\begin{proof}[Решение]
	...
	Край кусочно-гладкий, размерности компонент края в порядке по строкам: $0, 1, 1, 1, 2, 2, 2$ (каждая компонента - многообразие, размерность которых можно вычислить используя формулировки соотвествующих теорем о задании многообразий).
\end{proof}
...
\begin{proof}
	По условию, существует кривая $\gamma \subset M$, для которой $\vec{v}$ является вектором скорости. Зададим кривую $\tilde{\gamma}(t)=\gamma(\lambda t)$ и проверим, что вектор $\lambda \vec{v}$ явяляется ее вектором скорости. Действительно 
