...
\begin{example}[Вывод частной формулы из общей]
	...
	Воспользуемся цилиндрической системой координат $\varphi(x, \varphi) : U \to M$:
	\begin{equation*}
		\begin{cases}
		x = x, \\
		y = f(x)\cos \varphi, \\
	...
	\begin{equation*}
		...
		\begin{pmatrix}
			1 & 0 \\
			f'\cos \varphi & -f \sin \varphi \\
			f' \sin \varphi &  f \cos \varphi
		\end{pmatrix}
		\quad
		D \varphi^* = 
		\begin{pmatrix}
		1 & f'\cos \varphi & f' \sin \varphi \\
		0 & -f \sin \varphi & f \cos \varphi
		\end{pmatrix}
	\end{equation*}
...
Пусть $\varphi : U \to V$, $\psi : V \to W$ --- $C^1$-диффеоморфизмы, тогда $(\varphi \circ \psi)^{*} w =\varphi^{*} ( \psi^{*} w)$.
...
Пусть $w(x) = \sum\limits_{i=1}^{n} a_i(x) dx_i$ и $\varphi$ задано в виде

\begin{equation*}
	\begin{cases}
		x_1 = \varphi_1(y_1, \ldots, y_n) \\
		\ldots \\
		x_n = \varphi_n(y_1, \ldots, y_n)
	\end{cases}
\end{equation*}

тогда оператор переноса можно записать в координатном виде,

\begin{equation}
	\varphi^* w(y) = \sum\limits_{i=1}^{n} a_i(\varphi(y)) d\varphi_i(y)
\end{equation}
...
Свойства интеграла 1-формы
	...
	\begin{proof}
		...
		\begin{equation*}
			 \int\limits_{a}^{b} w(\psi(\psi^{-1}(\varphi(s)))) \langle \psi'(\psi^{-1}(\varphi(s))) \rangle (\psi^{-1} \circ \varphi)'(s) ds = \int\limits_{a}^{b} w(\varphi(s)) \langle \varphiэ(s) \rangle ds
		\end{equation*} 
	\end{proof}
...
\begin{lemma}
	...
	\begin{equation*}
		\int\limits_{\gamma} \Theta = \varphi(q) - \varphi(p)
	\end{equation*}
...
\begin{definition}
	Пусть $M \subseteq \mathbb{R}^n$ --- $k$-мерное $C^1$-гладкое ориентированное многообразие, а $w : M \to \Lambda^k(\mathbb{R}^n)$ --- непрерывная $k$-форма. Тогда, если $\varphi : U \subseteq \mathbb{R}^n \to M$ --- параметризация $M$, тогда интеграл $k$-формы по ориентированному $k$-мерному многообразию вычисляется как
	
	\begin{equation*}
		\int\limits_{M} w := \int\limits_{U} \varphi^* w
	\end{equation*} 
\end{definition}
